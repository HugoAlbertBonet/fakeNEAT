...
class Individual:
    def __init__(self):
        ...
        self.gen = [1 for _ in range(ConfigNEGA.hidden_layers)]
...
class NEGA:
    ...
    def __call__(self, save = True):
        ...
        if save:
            ...
            plt.savefig(f"./images/Simple_{ConfigNEGA.name_dataset}_hiddenlayers{ConfigNEGA.hidden_layers}_maxneurons{ConfigNEGA.max_neurons}_population_size{ConfigNEGA.population_size}_mutation_rate{ConfigNEGA.mutation_rate}_crossovers{ConfigNEGA.crossovers}_fitness{min_fitness}_test{min_fit_test}.png")

            torch.save(self.best_population[0].network, f"./models/Simple_{ConfigNEGA.name_dataset}_{min_fitness}.pt")
            with open("./models/genes.txt", "a") as f:
                f.write(f'Simple_{ConfigNEGA.name_dataset}_{min_fitness}.pt' + ": " + str(self.best_population[0].gen))
